...
import { Map as BoloMap } from '../src/map';
...
async function generatePreviews(): Promise<void> {
  ...
  const getTilePixels = (tx: number, ty: number): Buffer => {
  ...
  const getCachedTile = (tx: number, ty: number): Buffer => {
    const key = `${tx},${ty}`;
    if (!cache[key]) { cache[key] = getTilePixels(tx, ty); }
    return cache[key];
  };
  ...
  for (const mapFile of mapFiles) {
    ...
    try {
      ...
      const map = BoloMap.load(buffer);

      const TileStoringView = {
        onRetile(cell: any, tx: number, ty: number) {
          if (cell.mine && !cell.pill && !cell.base) { ty += 10; }
          cell.tile = [tx, ty];
        }
      };
      ...
      if (minX > maxX) { minX = 0; minY = 0; maxX = MAP_SIZE - 1; maxY = MAP_SIZE - 1; }
      ...
      for (let y = 0; y < cropHeight; y++) {
        for (let x = 0; x < cropWidth; x++) {
          ...
          if (mapX >= 0 && mapX < MAP_SIZE && mapY >= 0 && mapY < MAP_SIZE) {
            ...
            const tx = (cell as any).tile ? (cell as any).tile[0] : 0;
            const ty = (cell as any).tile ? (cell as any).tile[1] : 0;
